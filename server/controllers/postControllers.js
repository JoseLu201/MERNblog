    ...
    try {
        ...
        thumbnail.mv(path.join(__dirname, '..', '/uploads', newFilename), async (err) => {
            if(err) {
                ...
                if(!newPost) {
                    return next(new HttpError('Post couldtn be created', 422))
                }
...
const getPosts = async (req, res, next) => {
    try {
        const posts = await Post.find().sort({updatedAt: -1}) //recent ones first
...
const editPost = async (req, res, next) => {
    try {
        ...
        if(!updatedPost) {
            return next(new HttpError('couldtn update the post', 400))
        }
...
const deletePost = async (req, res, next) => {
    try {
        ...
        if(req.user.id == post.creator) {
            ...
            return next(new HttpError('post couldtn be deleted', 403))
